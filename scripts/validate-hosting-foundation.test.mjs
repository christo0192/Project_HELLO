 ...
 *  - current-state invariants unchanged; no .env baked; no AI attribution in
 *    changed files.
 *
...
const attributionPatterns = [
  /\bco-authored-by\b/i,
  /\bauthored by (claude|deepseek|gpt|codex|copilot|gemini|llama|openai)/i,
  /\bgenerated by (claude|deepseek|gpt|codex|copilot|gemini|llama|openai)/i,
  /\bgenerated with (claude|deepseek|gpt|codex|copilot|gemini|llama|openai)/i,
  /\bwritten by (claude|deepseek|gpt|codex|copilot|gemini|llama|openai)/i,
  /\bcreated by (claude|deepseek|gpt|codex|copilot|gemini|llama|openai)/i,
  /\b(claude|deepseek|gpt-4|gpt-5|codex|copilot)\s+generated/i,
  /\bai[- ]?generated/i,
];
const changedFiles = [
  "app/voice-livekit/requirements.txt",
  "app/voice-livekit/Dockerfile",
  "app/voice-livekit/docker-compose.yml",
  "app/voice-livekit/.dockerignore",
  ...
  "infra/hosting/CHECKLIST.md",
  "infra/hosting/vps-fallback/README.md",
  "infra/hosting/vps-fallback/systemd/voice-agent.service.example",
];
...
// ── attribution scan on changed files ─────────────────────────────────────
await checkAsync("no AI/model attribution in changed files", async () => {
  for (const file of changedFiles) {
    if (!(await exists(file))) continue;
    const content = await read(file);
    for (const pattern of attributionPatterns) {
      assert.doesNotMatch(content, pattern, `${file} contains attribution marker`);
    }
  }
});
