...
ok(detect && api && voice, "expected detect, deploy-api, deploy-voice jobs to exist");
...
// 4. Per-service secret separation (the core hardening ask)
...
// 5. Concurrency: top-level + per-service groups, never cancel in-flight
...
// 6. Immutable action pins — every `uses:` must be a full 40-hex commit SHA
...
// 7. Health / current-registration verification
...
console.log("deploy-fly workflow contract valid (11 property groups).");
