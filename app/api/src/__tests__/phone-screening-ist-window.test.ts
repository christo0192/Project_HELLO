...
import { helperLiteral, functionBody, MIGRATION_0042 } from './support/phone-migration.js';
...
describe('the DATABASE owns the window and the cap', () => {
  ...
  it('the predicate is 09:00 INCLUSIVE to 21:00 EXCLUSIVE, as the SQL says', () => {
...
describe('boundaries, on all seven days', () => {
  // 2026-08-17 is a Monday; the seven consecutive dates cover every weekday.
  const DAYS = [17, 18, 19, 20, 21, 22, 23];

  it('08:59:59 refused, 09:00:00 admitted, 20:59:59 admitted, 21:00:00 refused', () => {
    for (const day of DAYS) {
      expect(istWindowOpen(ist(2026, 8, day, 8, 59, 59))).toBe(false);
      expect(istWindowOpen(ist(2026, 8, day, 9, 0, 0))).toBe(true);
      expect(istWindowOpen(ist(2026, 8, day, 20, 59, 59))).toBe(true);
      expect(istWindowOpen(ist(2026, 8, day, 21, 0, 0))).toBe(false);
    }
  });

  it('every day is a calling day — there is no weekend skip', () => {
    for (const day of DAYS) {
      const noon = ist(2026, 8, day, 12, 0, 0);
    ...
    expect(functionBody('phone_next_window_open')).toContain('Every day is a calling');
  });
  ...
  it('IST midnight, not UTC midnight, rolls the day', () => {
    ...
    // And the mirror case: 2026-08-22T00:30:00Z is still 2026-08-22 in both.
    const utcMidnightish = new Date('2026-08-22T00:30:00.000Z');
    expect(istDate(utcMidnightish)).toBe('2026-08-22');
    // 06:00 IST — before the window opens.
    expect(istWindowOpen(utcMidnightish)).toBe(false);
  });

  it('istSecondsOfDay and istWallClock agree with the constructed instant', () => {
    const at = ist(2026, 8, 22, 20, 59, 59);
    expect(istWallClock(at)).toEqual({
      year: 2026, month: 8, day: 22, hour: 20, minute: 59, second: 59,
    });
...
describe('the next legal instant', () => {
  it('before today\'s open returns today\'s open', () => {
    const early = ist(2026, 8, 22, 6, 0, 0);
    expect(nextIstWindowOpen(early).getTime()).toBe(ist(2026, 8, 22, 9, 0, 0).getTime());
  });

  it('inside the window returns the instant itself', () => {
    const inside = ist(2026, 8, 22, 14, 3, 7);
    expect(nextIstWindowOpen(inside).getTime()).toBe(inside.getTime());
  });

  it('at or after close returns TOMORROW\'S open', () => {
    expect(nextIstWindowOpen(ist(2026, 8, 22, 21, 0, 0)).getTime())
      .toBe(ist(2026, 8, 23, 9, 0, 0).getTime());
    expect(nextIstWindowOpen(ist(2026, 8, 31, 23, 30, 0)).getTime())
      .toBe(ist(2026, 9, 1, 9, 0, 0).getTime());
  });
  ...
  it('20:59 plus a 120-second reconnect backoff lands OUTSIDE the window', () => {
    ...
    const drop = ist(2026, 8, 22, 20, 59, 0);
    ...
    expect(nextIstWindowOpen(afterBackoff).getTime()).toBe(ist(2026, 8, 23, 9, 0, 0).getTime());
  });
  ...
  it('the next-IST-DAY helper always skips to tomorrow, even mid-window', () => {
    ...
    const midday = ist(2026, 8, 22, 12, 0, 0);
    expect(nextIstDayWindowOpen(midday).getTime()).toBe(ist(2026, 8, 23, 9, 0, 0).getTime());
