/**
 * lib/scheduler.ts — the process's only owner of recurring background work.
 *
 * This module was `integrations/ashby/scheduler.ts`. It moved here because a
 * SECOND subsystem — authoritative-recording finalization — now needs the same
 * primitive, and that subsystem must keep converging while the Ashby runtime is
 * paused. A recording sweeper reached through an Ashby import path would be
 * Ashby-coupled by construction, which is exactly the dependency the canary
 * makes unacceptable.
 *
 * The move is mechanical with ONE behavioural correction. The claim that the
 * module "constructs nothing Ashby-specific" was FALSE: it emitted
 * `ashby_scheduler_tick`, `ashby_scheduler_tick_error`, and — the real problem
 * — the process-global gauge `ashby_scheduler_running`. Two schedulers in one
 * process would each write that gauge, so stopping one would report the other
 * as stopped. The metric name prefix is therefore per-instance, defaulting to
 * `ashby` so the existing suite and dashboards are untouched.
 *
 ...
 *
 * Nothing here is constructed when the owning runtime's gates are closed.
 */

import { createQueueRunner, nextPollDelayMs, type QueueRunnerHandle, type QueueHandler } from './queue/runner.js';
...
export interface LoopSchedulerOptions {
  ...
  /**
   * Metric-name prefix for this INSTANCE, e.g. `ashby` → `ashby_scheduler_tick`
   * and the `ashby_scheduler_running` gauge. Per-instance because the gauge is
   * process-global: two schedulers sharing a prefix would overwrite each
   * other's liveness. Defaults to `ashby` so the pre-existing Ashby metric
   * names and their suite are unchanged.
   */
  metricPrefix?: string;
...
/** Bound a caller-supplied metric prefix to a safe metric-name token. */
function safePrefix(raw: string | undefined): string {
  return typeof raw === 'string' && /^[a-z][a-z0-9_]{0,31}$/.test(raw) ? raw : 'ashby';
}

/**
 * Build the scheduler. Nothing is armed until `start()`.
 */
export function createLoopScheduler(options: LoopSchedulerOptions): LoopSchedulerHandle {
  const setTimer = options.setTimer ?? ((fn, ms) => setTimeout(fn, ms));
  const clearTimer = options.clearTimer ?? ((h) => clearTimeout(h as ReturnType<typeof setTimeout>));
  const random = options.random ?? Math.random;
  const now = options.now ?? Date.now;
  const prefix = safePrefix(options.metricPrefix);
  ...
  async function runTick(loop: SchedulerLoopConfig): Promise<void> {
    ...
    try {
      ...
      counter(`${prefix}_scheduler_tick`, 1, { loop: loop.name });
    } catch {
      ...
      counter(`${prefix}_scheduler_tick_error`, 1, { loop: loop.name });
  ...
  return {
    start(): void {
      ...
      gauge(`${prefix}_scheduler_running`, 1);
    },
    async stop(): Promise<void> {
      ...
      started = false;
      gauge(`${prefix}_scheduler_running`, 0);
    },
