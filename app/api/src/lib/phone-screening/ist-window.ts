 ...
 * bounds and fleet cap. Migration 0064 (cutoff re-asserted by 0085, then
 * ENDED by 0092) owns the reviewed, date-bounded temporary override; the
 * predicate below mirrors it.
 ...
 *
 * Mirror of `screening_v2.phone_temporary_247_until()`. Migration 0092 ended
 * the testing allowance by pulling this back to 2026-09-09 — a date that has
 * already elapsed — so every later IST date is governed by the permanent
 * 09:00–21:00 bounds above. The constant is kept (rather than the mechanism
 * deleted) so the mirror and the SQL stay the SAME fact: the 2026-09-07
 * reconciliation showed that when the two drift, the due-loop preflight —
 * which consults THIS mirror before admission ever reaches SQL — silently
 * decides alone. Change this only together with a migration that changes the
 * SQL function to the same date.
 */
