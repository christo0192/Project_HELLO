-- 0075 — preserve provenance when a candidate answers a later objective early.
-- The spoken exchange is written once for the currently owed key. Additional
-- contiguous keys are coverage attestations pointing at that same real answer;
-- no synthetic bot question is inserted into the transcript.
...
comment on function screening_v2.commit_phone_question_boundary_with_coverage is
  'Commits one real exchange and atomically advances contiguous future objectives already answered by that exchange without synthetic transcript turns.';
