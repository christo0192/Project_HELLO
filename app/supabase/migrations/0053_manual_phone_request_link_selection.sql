-- 0053 — manual phone requests must select a reusable application link.
--
-- A candidate can have several historical Ashby links whose engagement is
-- terminal. Ordering only by link.updated_at can select one of those rows and
-- return engagement_terminal even when a newer/reusable link exists. Prefer a
-- link with no engagement or a non-terminal engagement, then use the existing
-- recency ordering. This does not bypass any prerequisite or dial gate.
...
begin
  ...
  select l.id into v_link_id
    ...
    left join screening_v2.phone_engagements e
      on e.application_link_id = l.id
   where l.provider = 'ashby'
     and l.candidate_id = p_candidate_id
     and l.terminal_state is null
     and l.lifecycle not in ('completed', 'cancelled')
   order by
     case when e.id is null then 0
          when e.terminal_at is null then 1
          else 2 end,
     l.updated_at desc,
     l.id desc
   limit 1;
...
comment on function screening_v2.request_candidate_phone_call is
  'Recruiter-confirmed, idempotent manual request. Selects a reusable non-terminal Ashby application link before ensuring its engagement; it creates/adopts only the application-scoped engagement and never originates a provider call.';
