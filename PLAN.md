...
| **TST-07** | Accessibility tests: WCAG 2.1 AA on dashboard (axe-core plus manual assistive-technology checks). Candidate consent/call flow accessible | P0 | Frontend | — | M | Axe audit clean (automated WCAG 2.1 A/AA + best-practice in vitest/jsdom; colour-contrast deferred to manual). Keyboard focus assertions for available controls. Network isolation trap validates no unintended API calls. Lighthouse accessibility ≥ 90 (pending deployment). | Automated axe unit tests complete (101 tests, 50+ axe-assertion calls). Manual AT, contrast, reflow, real-browser, and call-flow accessibility remain external/dependent. Candidate consent blocked on GOV-08/Legal. Not a launch gate until all manual checks pass. |
